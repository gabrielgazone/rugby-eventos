...
""")
    ...
    if ('api' in st.session_state and 'atletas_sel' in st.session_state and
        ...
        if resultados_por_periodo:
            ...
            with _main_tabs[1]:
                _sub_campo = st.tabs(["🗺️ Campo de Rugby", "🎬 História do Jogo", "⚡ WCS"])
            with _main_tabs[2]:
...
<div style="background:linear-gradient(135deg,rgba(25,35,55,0.65) 0%,rgba(15,25,45,0.45) 100%);
  ...
  <strong style="color:white">{_tmx:.1f} {_unidade_jan}</strong>.
            ...
            with abas[7]:
                ...
                if _hist_cfg:
                    _hist_fl = float(_hist_cfg.get('fl', 100))
                    _hist_fw = float(_hist_cfg.get('fw', 70))
                else:
                    _hist_venue = st.session_state.get('venue', {})
                    _hist_fl = float(_hist_venue.get('length') or 100)
                    _hist_fw = float(_hist_venue.get('width') or 70)
                    st.info("💡 Configure o campo na aba **🗺️ Campo de Rugby** para usar as dimensões exatas do campo.")
                    ...
                    if not _hist_atletas_disp:
                        ...
                        if not _hist_coords:
                            ...
                            _cfg_aviso = (
                                ...
                                "**🗺️ Campo de Rugby** e configure o campo para "
                            ...
                            # ── Figura base (campo) ────────────────────────
                            _fig_hist = desenhar_campo_rugby_bonito(
                                ...
                                with _tac_tabs[1]:
                                    ...
                                    try:
                                        ...
                                        for _gi, _gframe in enumerate(_gap_frames):
                                            ...
                                            with _gcols[_gi]:
                                                _fig_gap = desenhar_campo_rugby_bonito(
                                                    field_length=_hist_fl, field_width=_hist_fw,
